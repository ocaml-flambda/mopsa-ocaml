...
def f(percent=100, global_options=None):
    ...
    for config_name, config_str in base_config.items():
        for option_name, option_str in options.items():
            ...
            s = f"{s}| {name1:20} | {name2:23} | speedup "
        ...
        try:
            for config_name, config_str in base_config.items():
                for option_name, option_str in options.items():
                    ...
                    print(f"| {t_noopt:13.3f}s ({errs_noopt:>3}) | {t_opt:8.3f}s ({gc_time:5.3f}s, {errs_opt:>3}) | {err:6.2f}% ", end="", flush=True)
            print("|")
    ...
    for config_name, config_str in base_config.items():
        for option_name, option_str in options.items():
            ...
            print(f"| {total_noopt[config_name, option_name]:19.3f}s | {total_opt[config_name, option_name]:22.3f}s | {err:6.2f}% ", end="", flush=True)
    print("|\n")
