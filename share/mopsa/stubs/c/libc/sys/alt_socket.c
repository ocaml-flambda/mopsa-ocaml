 ...
 * local:    void* f = _mopsa_find_file_resource(__fd);
 * requires: alive_resource(f, FileRes);
 * requires: __addr != NULL implies valid_ptr(__addr_len);
 * requires: null_or_valid_bytes(__addr, *__addr_len);
 *
 * case "valid-addr" {
 *   assumes: __addr != NULL;
 *   local:   void *f = new FileRes;
 *   local:   int fd = _mopsa_register_file_resource(f);
 *   assigns: ((char*)__addr)[0, *__addr_len);
 *   ensures: return == fd;
 * }
 *
 * case "null-addr" {
 ...
 * }
 */
int getpeername (int __fd, struct sockaddr* __addr, socklen_t *__restrict __len);
