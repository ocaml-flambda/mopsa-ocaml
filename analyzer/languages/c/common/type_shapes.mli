type type_shape =
  ...
  (** Block whose tag is below no-scan tag (i.e., a normal ocaml block value). If the
      ...
      shapes of its fields. Fittingly, in the case of [Some (t, shs)], the number of fields
      is known statically (i.e., the length of the list [shs]).

      To represent arrays (which are blocks with tag 0 at run time, but whose size is
      not statically known), there is a separate construtor, [Array sh], which keeps
      track of the shapes of the elements. *)
  | Array of type_shape
...
val serialize_extfun : extfun -> string
val deserialize_extfun : string -> extfun option
