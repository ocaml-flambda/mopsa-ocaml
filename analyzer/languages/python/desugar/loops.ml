...
let () =
  register_language_option "python" {
      ...
      spec = Clear opt_desugar_for_range;
      default = "enabled";
    };
  register_language_option "python" {
      ...
      spec = Clear opt_desugar_for_tuple;
      default = "enabled";
    }
...
module Domain =
  struct
    ...
    let exec stmt man flow =
      ...
      | S_py_for(target, ({ekind = E_py_object (addr, oe)} as iterable), body, orelse)
           when (not !opt_desugar_for_range || match akind addr, skind orelse with
                | A_py_instance {addr_kind =  A_py_class (C_builtin "range", _)}, S_block ([], _) -> false
                | _ -> true)
                && (not !opt_desugar_for_tuple || match akind addr, skind orelse with
                | Objects.Tuple.A_py_tuple _, S_block ([], _) -> false
                | _ -> true)
                  ->
