        ...
        let rec process_file lineno =
          try
            ...
            if is_comment l then
              ...
              ) entry.reachable_stmts then (* okay, statement reached *)
              Format.fprintf ocf "\027[38;5;%dm%s\027[0m@." (List.assoc "green" Debug.colors) l
            else
              begin match ExprSet.find_first_opt (fun e ->
                let r = e.erange in
                let start = get_range_start r in
                let stop = get_range_end r in
                get_pos_line start = lineno && lineno = get_pos_line stop) entry.reachable_exprs with
              | Some e ->
