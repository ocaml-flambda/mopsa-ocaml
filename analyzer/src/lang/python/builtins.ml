...
let mk_builtin_class_addr base cls =
  let name = mk_unname base cls.py_cls_var.orgname in
...
let mk_builtin_function_addr base fundec =
  let name = mk_unname base fundec.py_func_var.orgname in
  let range = builtin_range name in
  {
...
let rec parse_classes modl stmt =
  ...
  | S_py_class(cls) ->
    ...
    let base = mk_unname modl cls.py_cls_var.orgname in
