...
module Domain =
  struct
    ...
    let exec zone stmt man flow =
      ...
      | S_py_for(target, ({ekind = E_py_object (addr, oe)} as iterable), body, orelse) when match akind addr with
                                                                                            | A_py_instance {addr_kind =  A_py_class (C_builtin "range", _)} -> false
                                                                                            | _ -> true ->
