...
let partition_flow man (l: partitioning_location_label) (assoc: (son_labels * 'a Flow.flow) list) annot =
  let fmap =
    List.fold_left (fun map (part_key, f) ->
        ...
        begin
          try
            ...
            if ListExt.mem_compare compare_son_labels son part_keys then
              match remove_partitition_history l h with
              | None -> FlowMap.add (tk_org) env map
              | Some h' -> FlowMap.add (T_part(h', tk_org)) env map
            else map
          with
      ...
      | T_part (h, tk_org) ->
        begin
          try
            ...
            match remove_partitition_history l h with
            | None ->
              SonLabelMap.add i (FlowMap.add (tk_org) env cur_flow_i) map
            | Some h' ->
...
struct
  ...
  type _ domain += D_universal_cf_simple_partitioning : unit domain

  let id = D_universal_cf_simple_partitioning
  let name = "universal.partitioning.cf.cf_simple_partitioning"
  let identify : type a. a domain -> (unit, a) eq option =
    function
    | D_universal_cf_simple_partitioning -> Some Eq
    | _ -> None
  ...
  let cf_partitioning_waiting flow =
    try
      Some (Flow.get_annot A_cf_part flow, Flow.rm_annot A_cf_part flow)
    with
    | Not_found -> None
  ...
  let rec exec zone stmt man flow =
    ...
    | S_if (cond, s_then, s_else) ->
      begin
        let () = debug "Got the if statement" in
        match cf_partitioning_waiting flow with
        (* Control flow partitioning should be done on this if
           statement. *)
        | Some (l, flow) ->
          let () = debug "Found label %d waiting in annotations" l in
          let range = srange stmt in

          let flow_then = man.exec (mk_assume cond range) flow |>
                           man.exec s_then
          in
          let annot_then = Flow.get_all_annot flow_then in
          let flow_else = man.exec (mk_assume (mk_not cond range) range)
              (Flow.set_all_annot annot_then flow) |>
                          man.exec s_else
          in
          let annot_else = Flow.get_all_annot flow_else in
          let f = partition_flow man l [(0, flow_then); (1, flow_else)] annot_else in
          Some (f |> Post.of_flow)
    ...
    | S_cf_part_start(i) ->
      (* Starting a new control flow partition on next statement,
          ...
          exist) by [i] *)
      let () = debug "Starting a new partitioning on label %d" i in
      let annot' = i in
