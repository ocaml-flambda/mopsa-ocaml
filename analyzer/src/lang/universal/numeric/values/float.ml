...
let name = "universal.numeric.values.float"
...
struct
  ...
  let of_constant = function
    | C_float i -> Nb (I.cst i)
    | C_float_interval (i1,i2) -> Nb (I.of_float i1 i2)
    | _ -> top

  let of_bool = function
    | Some true -> Nb (I.cst 1.)
    | Some false -> Nb (I.cst 0.)
    | None -> Nb (I.of_float 0. 1.)


  let to_int a =
    bot_absorb1 (fun f ->
        let {I.lo; up} = I.round f in
        Int.Value.of_float lo up
      ) a
  ...
  let of_int_interval (iitv: Int.Value.t) : t =
