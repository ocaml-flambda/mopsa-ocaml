...
let fwd_unop op a =
  ...
  | O_minus T_float -> bot_lift1 I.neg a
...
let fwd_binop op a1 a2 =
  match op with
  | O_plus T_float  -> bot_lift2 I.add a1 a2
  | O_minus T_float -> bot_lift2 I.sub a1 a2
  | O_mult T_float  -> bot_lift2 I.mul a1 a2
  | O_div T_float   -> bot_absorb2 I.div a1 a2
  | O_pow   -> bot_lift2 I.pow a1 a2
  | O_log_or   -> bot_lift2 I.log_or a1 a2
  | O_log_and  -> bot_lift2 I.log_and a1 a2
  | O_mod T_float   -> bot_absorb2 I.fmod a1 a2
  | _     -> top
...
let bwd_unop op abs rabs =
  try
    ...
    let aa = match op with
      ...
      | O_minus T_float -> bot_to_exn (I.bwd_neg a r)
      | _ -> assert false
    in
    Nb aa
  with Found_BOT ->
...
let bwd_binop op a1 a2 r =
  try
    ...
    let aa1, aa2 =
      match op with
      | O_plus T_float  -> bot_to_exn (I.bwd_add a1 a2 r)
      | O_minus T_float -> bot_to_exn (I.bwd_sub a1 a2 r)
      | O_mult T_float  -> bot_to_exn (I.bwd_mul a1 a2 r)
      | O_div T_float   -> bot_to_exn (I.bwd_div a1 a2 r)
      | O_mod T_float   -> bot_to_exn (I.bwd_rem a1 a2 r)
      | _ -> assert false
    in
    Nb aa1, Nb aa2
  with Found_BOT ->
