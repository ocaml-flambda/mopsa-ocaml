...
struct
  ...
  let widening (u: t) (v: t) =
    ...
    (* let stable_support = RegExp.is_bottom auto_algebra widening_position in *)
    ...
    let rec merge_and_rename_both absu absv (envu: RegexpPartition.t)  envv todosu nlinks =
      if Linker.S.is_empty todosu then (absu, absv, envu, envv, nlinks)
      else
        ...
        let absu = Numerical.merge absu u_newn (List.map snd to_merge_u) in
        let absv = Numerical.merge absv v_newn (List.map snd to_merge_v) in
        ...
        merge_and_rename_both absu absv envu envv todosu nlinks
    in

    (* let () = debug "mark III%!" in
     * let () = debug "envu1: %a" RegexpPartition.print u.env in
     * let () = debug "envv1: %a" RegexpPartition.print v.env in *)

    let absu, absv, envu, envv, nlinks = merge_and_rename_both
        u.numeric
        v.numeric
        u.env
        v.env
        ...
        []
    in
    (* let () = debug "mark IV%!" in *)
    let comp_support_u = RegExp.compl (*auto_algebra*) u.support in
    let comp_support_v = RegExp.compl (*auto_algebra*) v.support in

    (* let envv = List.filter (fun (_, vn) -> List.exists (fun (_,(_, vn'),_) -> vn = vn') nlinks ) envv in *)

    (* let () = debug "envu: %a" RegexpPartition.print envu in
     * let () = debug "envv: %a" RegexpPartition.print envv in *)

    let envu, envv, absu, absv, reste =
      List.fold_left (fun (envu, envv, absu, absv, reste ) (ue, un) ->
          try
            ...
            && RegExp.leq (*auto_algebra*) diffvu comp_support_u then
              ...
              Numerical.renaming_list [un, newzn] absu, Numerical.renaming_list [vn, newzn] absv,
              new_reste
            else
              (newz, newzn)::envu, (newz,newzn)::envv,
              Numerical.renaming_list [un, newzn] absu |> Numerical.forget newzn,
              Numerical.renaming_list [vn, newzn] absv |> Numerical.forget newzn,
              new_reste
          with
          | Not_found ->
            if RegExp.leq (*auto_algebra*) ue comp_support_v then
              ((ue, un) :: envu, envv, absu, absv, reste)
            else
              ((ue, un) :: envu, envv, absu |> forget un, absv, reste)

        ) ([], [], absu, absv, widening_position) envu
    in

    (* let () = debug "mark V%!" in
     * let () = debug "envu3: %a" RegexpPartition.print envu in
     * let () = debug "envv3: %a" RegexpPartition.print envv in
     *
     * let () = debug "absu: %a" Numerical.print absu in
     * let () = debug "absv: %a" Numerical.print absv in *)

    let nenv = envu in
    let nabs = Numerical.widening_different_support absu absv (List.map snd envv) in
    {
      shape = nshape;
      support = nsupport;
      classes = neqclass;
  ...
  let bottom (sa: TA.sigma_algebra)=
    ...
    let re_algebra = automata_algebra_on_n sa in
    {
      ...
      classes = [];
      env = [];
      numeric = Numerical.bottom (Environmentext.empty);
    }
