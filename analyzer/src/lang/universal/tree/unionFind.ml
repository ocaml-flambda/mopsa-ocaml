let debug fmt =
  Format.kasprintf (fun str ->
      Format.printf "\027[1;3m[Debug]\027[0m @.@[%s@]@.@." str
    ) fmt
...
module Make(S : SIG.COMPARABLE) = struct
  ...
  let print_map fmt (ss : SS.t SM.t) : unit =
    Format.fprintf fmt "{%a}"
      (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ",") (fun fmt (x,y) -> Format.fprintf fmt "%a->%a" S.print x print_state_set y)) (SM.bindings ss)

  let print_bare fmt u =
    Format.fprintf fmt "{@[<v>%a@]}" (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt "@,") (fun fmt (x,y) -> Format.fprintf fmt "%a -> %a" S.print x S.print y)) (SM.bindings u)
  ...
  let to_map u =
    let sm = SM.fold (fun k _ (acc,nu) ->
        let a,nu = find k nu in
        if SM.mem a acc then
          let y = SM.find a acc in
          (acc
          |> SM.remove a
          |> SM.add a (SS.add k y)),nu
        else
          (SM.add a (SS.singleton k) acc,nu)
      ) u (SM.empty,u) in
    (* let () = debug "@[<v>to_map : %a@,%a@]" print_bare u print_map (fst sm) in *)
    sm
  ...
  let nb_class (u : t) =
    u |> to_list |> fst |> List.length

  let print (fmt : Format.formatter) (u : t) : unit =
    let sm,_ = to_map u in
    Format.fprintf fmt "{%a}" (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ",") (fun fmt (_,k) -> Format.fprintf fmt "{%a}" (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt ",") S.print) (SS.elements k))) (SM.bindings sm)

  let from_list l =
    let all = List.fold_left (fun acc x ->
