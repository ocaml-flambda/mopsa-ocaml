 ...
 *   include Nonrel
 *   (\* module Lifted = Framework.Sig.Abstraction.Simplified.MakeDomain(Nonrel)
 *    * include Lifted *\)
 *   let alarms = []
 ...
 *    *         Lifted.exec {stmt with skind = S_assign(x, ee)} man flow
 *    *       )
 *    *
 *    *   | _ ->
 *    *     Lifted.exec stmt man flow
 ...
 *
 *
 *   let ask : type r. ('a, r) query -> ('a, _) man -> 'a flow -> r option =
