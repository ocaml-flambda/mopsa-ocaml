...
(** Compose domains hierarchically in a stack.

    As for {!Composers.Parallel}, lattice operations
    ([bottom], [top], [leq], etc.) are defined pointwise.
    For [exec], [eval] and [ask], we return the result of the first stack
    layer giving an non-empty answer.
*)
...
struct
  ...
  let eval exp man ctx gabs =
    let head_ev = Head.eval exp (head_man man) (tail_man man) ctx gabs in
    match head_ev with
