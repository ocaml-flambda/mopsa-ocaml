...
let compare_alarm_report_chain = TypeExt.mk_compare_chain (fun a1 a2 -> compare a1 a2)

let compare_alarm_report = TypeExt.compare compare_alarm_report_chain

let compare_alarm a1 a2 =
  if a1 == a2 then 0
  else Compare.compose [
      (fun () -> compare_alarm_kind a1.alarm_kind a2.alarm_kind);
      (fun () -> compare_alarm_report a1.alarm_report a2.alarm_report);
...
let pp_report_chain = TypeExt.mk_print_chain (fun fmt alarm -> failwith "Pp: Unknown alarm")

let pp_alarm_report = TypeExt.print pp_report_chain
...
let pp_alarm fmt alarm =
  ...
  Format.fprintf fmt "%a: %a %a@\n%a"
    ...
    pp_alarm_kind alarm.alarm_kind
...
let register_alarm_report info =
  TypeExt.register info compare_alarm_report_chain pp_report_chain



(** {2 Empty alarm report} *)
(** ********************** *)

type alarm_report += AR_empty
...
let mk_alarm kind ?(report=AR_empty) ?(level=WARNING) ?(cs=Callstack.empty) range =
  {
    alarm_kind = kind;
    alarm_report = report;
    alarm_level = level;
