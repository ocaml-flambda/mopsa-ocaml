...
struct
  ...
  let init prog man sman flow =
    let flow1 =
      match S1.init prog (s1_man man) (s1_sman man sman) flow with
      | None -> flow
      | Some flow -> flow
    in
  ...
  let eval zone =
    ...
    | true, false ->
      ...
      (fun exp man sman flow ->
         f exp (s1_man man) (s1_sman man sman) flow
      )
    ...
    | true, true ->
      ...
      (fun exp man sman flow ->
         match f1 exp (s1_man man) (s1_sman man sman) flow with
         | Some evl -> Some evl
  ...
  let ask query man sman flow =
    let reply1 = S1.ask query (s1_man man) (s1_sman man sman) flow in
