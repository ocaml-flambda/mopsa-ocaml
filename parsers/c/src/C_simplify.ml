...
let simplify_func ctx (f:func) =
  ...
  let rec simplify_expr ((e,t,r):expr)
    ...
    | E_conditional (e1,e2,e3) ->
       ...
       if is_void t then
         let cond = S_if (e1,
                          simplify_expr_stmt e2 |> make_block,
                          simplify_expr_stmt e3 |> make_block), r
         in
   ...
   | E_compound_literal i ->
      let before1, i, after1 = simplify_init i in
      let tmp = make_temp ctx r f t in
      let tmp_var = E_variable tmp, t, r in
      let create = S_local_declaration tmp, r in
      tmp.var_init <- Some i;
  ...
  and simplify_expr_stmt ((e,t,r):expr) : statement list =
    match e with
   ...
   | E_comma (e1,e2) ->
      let b1 = simplify_expr_stmt e1 in
      let b2 = simplify_expr_stmt e2 in
      b1@b2
   ...
   | E_statement b ->
      List.concat (List.map simplify_stmt b.blk_stmts)
    ...
    | I_init_list (l1,opt) ->
       let before, l, after =
         List.fold_left (fun (before,l,after) i ->
             let before', i', after' = simplify_init i in
  ...
  and simplify_stmt ((s,r):statement) : statement list =
    match s with
    ...
    | S_expression e ->
       scope_temp r (simplify_expr_stmt e)
    ...
    | S_for (b1,e2,e3,b4) ->
       ...
       and e3 = match e3 with
         | None -> None
         | Some ((_,t3,r3) as e3) ->
            Some (as_expr_stmt (simplify_expr_stmt e3) t3 r3)
       in
  ...
  and simplify_block (b:block) : block =
    make_block (List.concat (List.map simplify_stmt b.blk_stmts))
  ...
  | Some body -> f.func_body <- Some (simplify_block body |> resolve_scope)
  | None -> ()
